...
def load_grid(grid_str):
    """ np.array() creates an ndarray from the grid, which is split 
    into lines, then numbers and finally integers via list comprehension. """
    return np.array([[int(num) for num in line.split()] for line in grid_str.strip().split("\n")])

def horizontal_product(grid, n):
...
def left_to_right_diagonal(grid):
    """ calculate the maximum number of diagonals based on the grid dimensions, and 
    pad shorter ones with zeros. """
    (y, x) = np.shape(grid)
    nx, ny = min(x, y), x + y - 1
...
def largest_grid_product(n, grid_str):
    grid = load_grid(grid_str)
    horizontal_big = horizontal_product(grid, n)
    """ np.transpose() switches the row and column indices of a 2d array. """
    vertical_big = horizontal_product(grid.transpose(), n)
    diagonal_left_big = horizontal_product(left_to_right_diagonal(grid), n)
    """ np.fliplr flips a 2d array in a left-right direction; columns are preserved, 
    but appear in a different order. """
    diagonal_right_big = horizontal_product(left_to_right_diagonal(np.fliplr(grid)), n)
    return int(max(horizontal_big, vertical_big, diagonal_left_big, diagonal_right_big))
...
if __name__ == "__main__":
    ...
    """
    print(largest_grid_product(4, grid_str))
