...
def _count_word_length(num):
    if num < 10:
        ...
        return len(["twenty", "thirty", "forty", "fifty", "sixty", "seventy", "eighty", "ninety"][num // 10 - 2]) + _count_word_length(num % 10)
    elif num < 1000:
        ...
        number. """
        return _count_word_length(num // 100) + len("hundred") + (0 if num % 100 == 0 else len("and") + _count_word_length(num % 100))
    else:
...
def number_letters_count(n):
    ...
    for i in range(1, n + 1):
        total_letters += _count_word_length(i)
