...
def factorial_digit_sum(n):
    """ combine length lemma from #16 with Stirling's asympotic approximation for 
    the length of a factorial. math.floor() rounds a number down to the nearest 
    integer. math.pi returns the constant value of pi, which is used as a scaling 
    factor, as is Euler's number, e, via math.e. """
    ...
    for i in range(2, n + 1):
        carry = 0
        """ for each digit, compute its product with the current i and add the 
        carry from the previous calculation. """
