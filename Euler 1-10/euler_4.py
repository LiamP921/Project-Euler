...
def _is_palindrome(x):
    ...
    head, tail = x, 0
    """ ensure that only half of the digits are processed, as the entire number 
    isn't needed to determine palindromicity. """
    while head > tail:
    ...
    """ If x has an odd number of digits, head will be one shorter than tail. 
    Head must equal tail without the last digit for the number to be palindromic. 
    If the number of digits in x is even, head and tail should be equal for the number 
    to be palindromic. """
    return head == tail or head == tail // 10

def _modular_inverse(x, modulus_power_of_10):
    """ describes the integer product ax which is congruent to 1 with respect 
    to the modulus m.(i.e. ax ≡ 1 (mod m)) """
